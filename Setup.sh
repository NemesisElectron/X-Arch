...
| Cloudflare One Client,          | | monitoring, tweaking, experimenting. |
| Mission Center,                 | +--------------------------------------+
| OBS Studio,                     |
| Winetricks,                     |
| OCCT,                           |
...
+---------------------------------+
+-----------------------------------------------------------------+
| Select cache type [RAM] or [SSD] without square brackets,       |
| Select drive type [NVME] or [SATA] without square brackets,     |
| Select processor type [INTEL] or [AMD] without square brackets. |
+-----------------------------------------------------------------+
"
read -p "Cache> " CACHE
if [ $CACHE != RAM ] && [ $CACHE != SSD ]; then
	exit
fi
read -p "Drive> " DRIVE
if [ $DRIVE != NVME ] && [ $DRIVE != SATA ]; then
	exit
fi
read -p "Processor> " PROCESSOR
if [ $PROCESSOR != INTEL ] && [ $PROCESSOR != AMD ]; then
	exit
fi
clear
cd /tmp/X-Arch
systemd-inhibit --mode=block --what=idle sleep 1d &
systemd-inhibit --mode=block --what=sleep sleep 1d &
git clone --branch main --recurse-submodules https://github.com/morrownr/rtw89.git
sudo dash ./Files/CachyOS.sh
sudo cachyos-rate-mirrors
sudo pacman --sync --noconfirm --refresh --sysupgrade
for PACKAGE in steam firefox mpv code kate partitionmanager plasma-systemmonitor konsole kitty protontricks sddm btop micro vim proton-cachyos proton-cachyos-slr wine-cachyos-opt wine-staging wine wine-gecko wine-mono fish linux-cachyos linux-cachyos-lts; do
...
sudo pacman --sync --noconfirm --needed firefox-developer-edition vlc davinci-resolve gimp audacity vscodium lsfg-vk goverlay lact gparted mission-center obs-studio winetricks occt hardinfo2 alacritty plasma-login-manager wine-cachyos mangohud
sudo pacman --sync --noconfirm --needed speech-dispatcher cloudflare-warp-bin xorg-xrandr plasma-foreground-booster dmemcg-booster bpftune-git yay downgrade dkms sbctl clang intel-compute-runtime rocm-opencl-runtime intel-ucode amd-ucode linux-cachyos-rc linux-cachyos-rc-headers
sudo pacman --sync --noconfirm wine-cachyos
sudo tar --extract --file ./Files/CachyOS.tar --directory /
...
sudo rm --force /usr/bin/sh
sudo rm --force /usr/bin/rbash
sudo rm --force --recursive /boot/loader
sudo rm --force --recursive /boot/EFI
sudo ln --force --symbolic /usr/bin/dash /usr/bin/sh
sudo ln --force --symbolic /usr/bin/dash /usr/bin/rbash
...
sudo cp --force ./Files/99-AMDGPU-Overdrive.conf /etc/modprobe.d/99-amdgpu-overdrive.conf
sudo cp --force ./Files/Linux-CachyOS-RC.conf /boot/loader/entries/linux-cachyos-rc.conf
sudo cp --force ./Files/Loader.conf /boot/loader/loader.conf
sudo chmod u+rwx /home/cachyos.sh
sudo chmod u+rwx /usr/bin/wine
sudo chmod u+rwx /etc/shells
sudo chmod u+rwx /etc/hosts
sudo chmod u+rwx /etc/environment
sudo chmod u+rwx /etc/mkinitcpio.conf
sudo chmod u+rwx /etc/systemd/system/cachyos.service
sudo chmod u+rwx /etc/systemd/timesyncd.conf
sudo chmod u+rwx /etc/systemd/resolved.conf
sudo chmod u+rwx /etc/modprobe.d/99-amdgpu-overdrive.conf
sudo chmod u+rwx /etc/modprobe.d/usb_storage.conf
sudo chmod u+rwx /etc/modprobe.d/rtw89.conf
sudo chmod u+rwx /boot/loader/entries/linux-cachyos-rc.conf
sudo chmod u+rwx /boot/loader/loader.conf
...
if [ $DRIVE != NVME ]; then
	sudo sed --in-place "s#nvme0n1#sda#" /home/cachyos.sh
	sudo sed --in-place "s#nvme0n1p#sda#" /boot/loader/entries/linux-cachyos-rc.conf
fi
if [ $PROCESSOR != INTEL ]; then
	sudo sed --in-place "s#intel#amd#" /boot/loader/entries/linux-cachyos-rc.conf
fi
...
yay --sync --noconfirm --needed reshade-shaders-git usb_oc-dkms
...
for LINK in firefox-developer-edition vlc DaVinciResolve gimp audacity vscodium lsfg-vk-ui io.github.benjamimgois.goverlay io.github.ilya_zlobintsev.LACT gparted com.cloudflare.WarpTaskbar io.missioncenter.MissionCenter com.obsproject.Studio winetricks occt hardinfo2 Alacritty; do
...
mkdir --parents ~/.wine/drive_c/ProgramData
mkdir --parents ~/.wine/drive_c/users/$USER/AppData/Local
mkdir --parents ~/.wine/drive_c/users/$USER/AppData/LocalLow/Microsoft
...
cp --force ./Files/Accepted-TOS.txt ~/.local/share/warp/accepted-tos.txt
chmod u+rwx $PROFILE/user.js
chmod u+rwx ~/.config/VSCodium/User/settings.json
chmod u+rwx ~/.config/lsfg-vk/conf.toml
chmod u+rwx ~/.config/goverlay/goverlay.conf
chmod u+rwx ~/.config/vkBasalt-overlay/configs/default.conf
chmod u+rwx ~/.config/vkBasalt-overlay/default_config
chmod u+rwx ~/.config/vkBasalt-overlay/shader_manager.conf
chmod u+rwx ~/.config/vkBasalt-overlay/vkBasalt.conf
chmod u+rwx ~/.config/MangoHud/MangoHud.conf
chmod u+rwx ~/.config/powerdevilrc
chmod u+rwx ~/.config/powermanagementprofilesrc
chmod u+rwx ~/.local/share/warp/accepted-tos.txt
warp-cli disconnect
...
rm --force --recursive ~/.wine/drive_c/ProgramData/"Package Cache"
rm --force --recursive ~/.wine/drive_c/users/$USER/AppData/Local/cache
rm --force --recursive ~/.wine/drive_c/users/$USER/AppData/Local/Temp
rm --force --recursive ~/.wine/drive_c/users/$USER/AppData/LocalLow/Microsoft/CryptnetUrlCache
rm --force --recursive ~/.wine/drive_c/users/Public
rm --force --recursive ~/.cache
if [ $CACHE != RAM ]; then
	mkdir --parents ~/.cache
fi
ln --force --symbolic /tmp ~/.cache
ln --force --symbolic ~/.cache ~/.wine/drive_c/windows/temp
ln --force --symbolic ~/.cache ~/.wine/drive_c/ProgramData/"Package Cache"
ln --force --symbolic ~/.cache ~/.wine/drive_c/users/$USER/AppData/Local/cache
ln --force --symbolic ~/.cache ~/.wine/drive_c/users/$USER/AppData/Local/Temp
ln --force --symbolic ~/.cache ~/.wine/drive_c/users/$USER/AppData/LocalLow/Microsoft/CryptnetUrlCache
ln --force --symbolic ~/.wine/drive_c/users/$USER ~/.wine/drive_c/users/Public
winetricks --force --unattended alldlls=default
winetricks --force --unattended steam physx shockwave
winetricks --force --unattended vkd3d dxvk_async dxvk_nvapi dsoal openal lavfilters ffdshow ogg powershell dinputto8
...
cp --force ./Files/DXVk.conf ~/.wine/drive_c/windows/dxvk.conf
cp --force ./Files/Hosts ~/.wine/drive_c/windows/system32/drivers/etc/hosts
chmod u+rwx ~/.wine/drive_c/windows/system32/lossless.dll
chmod u+rwx ~/.wine/drive_c/windows/syswow64/ddraw.dll
chmod u+rwx ~/.wine/drive_c/windows/dxvk.conf
chmod u+rwx ~/.wine/drive_c/windows/system32/drivers/etc/hosts
